...
class DoubleLinkedList<T> {
  ...
  void insertTailNode(dynamic data) {
    ...
    if (isEmpty()) {
      // Jika list kosong, tambahkan node menjadi head dan tail
      head = newNode;
      tail = newNode;
    } else {
      // Jika list tidak kosong, cari node terakhir dan tambah node baru
      tail!.next = newNode;
      newNode.prev = tail;
...
void main() {
  ...
  print("Sebelum menambah node di akhir:");
  dll.printList();

  // Menambahkan node baru di akhir
  dll.insertTailNode("Kuning");

  print("\nSetelah menambah node di akhir:");
  dll.printList();
}
